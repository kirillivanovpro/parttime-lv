...
const CATEGORY_CONFIG: Record<ListingCategory, { bg: string; icon: React.ReactNode }> = {
  cleaning: {
  ...
  garden: {
    ...
    icon: (
      <svg className="w-8 h-8 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364-.707-.707M6.343 6.343l-.707-.707m12.728 0-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
      </svg>
    ),
  },
  moving: {
...
export default function CategoryGrid({ t }: CategoryGridProps) {
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
      {CATEGORIES.map((cat) => {
        const { bg, icon } = CATEGORY_CONFIG[cat]
        return (
          <Link
            key={cat}
            href={`/category/${cat}`}
            className={`flex flex-col items-center gap-3 p-4 rounded-xl transition-all duration-200 cursor-pointer ${bg} group`}
          >
